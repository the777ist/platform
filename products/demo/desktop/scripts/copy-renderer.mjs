...
const SRC = path.join(desktopDir, "..", "app", "dist"); // products/demo/app/dist
const DEST = path.join(desktopDir, "renderer"); // products/demo/desktop/renderer
  ...
  try {
    ...
    console.error(
      `[copy-renderer] Missing ${path.join(SRC, "index.html")}.\n` +
        `Run the web export first: turbo run export:web --filter=*demo-app ` +
