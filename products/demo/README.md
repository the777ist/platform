...
pnpm --filter @sevenfold/demo-api dev         # FastAPI (port = 8000 + 10·portIndex)
pnpm --filter @sevenfold/demo-app dev         # Expo (web on :8081, QR for device)
```
...
Replace `app/assets/brand/source.svg`, run `pnpm --filter @sevenfold/demo-app brand:gen`,
...
```bash
pnpm --filter @sevenfold/demo-app test        # Jest + RNTL
pnpm --filter @sevenfold/demo-api test        # pytest against real Postgres
pnpm --filter @sevenfold/demo-app exec playwright test   # web E2E (full local stack)
