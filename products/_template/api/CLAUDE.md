...
5. **openapi** — `pnpm turbo run openapi --filter=@sevenfold/template-api` (or
   `uv run python -m template_api.export_openapi`).
6. **typegen** — `pnpm turbo run build --filter=@sevenfold/template-api-client`; commit
   the regenerated client. CI drift check fails a stale regen.
7. **hook** — use the generated TanStack options/mutations from
   `@sevenfold/template-api-client`.
