...
One-off run (staging):
    fly machine run --app sevenfold-template-api-stg \\
      registry.fly.io/sevenfold-template-api-stg:latest \\
      python -m template_api.tasks prune-push-tokens

Scheduled (daily) machine — Fly's built-in scheduler:
    fly machine run --app sevenfold-template-api-stg --schedule daily \\
      registry.fly.io/sevenfold-template-api-stg:latest \\
      python -m template_api.tasks prune-push-tokens
