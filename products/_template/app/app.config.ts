...
const config: ExpoConfig = {
  ...
  ios: { bundleIdentifier: "com.sevenfold.template" },
  android: {
    package: "com.sevenfold.template",
    adaptiveIcon: {
  ...
  plugins: [
    ...
    [
      // Build-time half of Sentry (runtime init lives in @sevenfold/core sentry.ts):
      // source-map upload + native symbolication. SENTRY_AUTH_TOKEN is a BUILD env
      // var (EAS secret) — never committed.
      "@sentry/react-native/expo",
      {
        organization: "sevenfold", // the org (Sentry org created on infra day)
        project: "sevenfold-template", // Sentry project slug (created on infra day)
      },
    ],
  ],
