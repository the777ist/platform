# CLAUDE.md — template product

Agent context for `products/template`. Monorepo-wide conventions live in the root
...
`product.json` holds `{"name": "template", "portIndex": i}`. All local ports derive
...
Infra names derive from the PRODUCT name: Fly `sevenfold-template-api-stg|prod`,
Supabase projects `sevenfold-template-stg|prod`, Sentry `sevenfold-template`, EAS project
...
- Committed `app/.env.development/.staging/.production` carry PUBLISHABLE values only
  (`EXPO_PUBLIC_*`); server secrets live in `api/.env` (gitignored, template at
  `.env.example`) and each platform's native store.
- Realtime: the api broadcasts `invalidate` on the `template:realtime` channel after
