...
pnpm --filter @sevenfold/template-api dev         # FastAPI (port = 8000 + 10·portIndex)
pnpm --filter @sevenfold/template-app dev         # Expo (web on :8081, QR for device)
```
...
- Shared components: `@sevenfold/ui` (workbench: `pnpm --filter @sevenfold/ui storybook`)
...
Replace `app/assets/brand/source.svg`, run `pnpm --filter @sevenfold/template-app brand:gen`,
...
```bash
pnpm --filter @sevenfold/template-app test        # Jest + RNTL
pnpm --filter @sevenfold/template-api test        # pytest against real Postgres
pnpm --filter @sevenfold/template-app exec playwright test   # web E2E (full local stack)
