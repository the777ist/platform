...
- **UI** — every component (existing and new), every `@the777incident/ui` primitive (`packages/ui/src/components/ui/*` — copied-in source you OWN; shadcn model) you'll compose with, every **semantic token** used (`bg-background`, `bg-primary`, `text-foreground`, `border-border`, … — NEVER raw hex; brand is a token _mode_), every cross-target difference (iOS / Android / web / desktop), light + dark, every responsive (web/tablet) breakpoint that matters, every loading / error / empty state.
...
### Performance & scale — size every data surface

Edge cases catch correctness bugs; this catches the ones that only show up **at volume**. Inherit the architecture's `## Scale, performance & extensibility` section when one exists; derive it directly otherwise. For every DB / query / mutation surface the plan touches, state:

- **Index** — the exact index the query's filter/sort needs (or confirm the UUIDv7 time-ordered PK already covers the cursor keyset). **Flag any filter/sort with no supporting index.**
- **N+1** — the service loads related rows in one query/join, never per-row in a loop.
- **Pagination bounds** — a max page size, no unbounded `SELECT`; **cursor keyset** (not OFFSET) on every `list_*`.
- **Payload size** — the DTO returns only the columns the surface needs (no over-fetch); large fields lazy-loaded.
- **Caching** — TanStack Query `gcTime`/staleness for the new query; whether a hot read wants longer caching or an edge layer.
- **Broadcast fan-out** — for each mutation that broadcasts, whether a high-frequency case would storm invalidations (and whether to coalesce/debounce).
- **Inline vs. job** — anything heavy (large fan-out, transcode, third-party batch) goes to a **background job / Fly machine**, not the request path.

Proportional: a thin CRUD surface is a line or two; a list / feed / search / media / real-time surface gets the full treatment. Each real concern lands in `## Backend changes` and — where testable — an assertion in `## Test plan` (e.g. "`list_comments` uses the `(post_id, created_at)` index"; "no N+1 — one query for N comments, asserted via query count").
...
The layers involved (Expo Router screen → `@the777incident/ui` component → generated-client hook → FastAPI router → service → SQLModel/Supabase → broadcast). A textual flow diagram (Mermaid where it clarifies). Data shapes (Pydantic DTOs / SQLModel rows / generated-client types / the occasional Zod form schema). State machines if any.
...
New / modified components and the `@the777incident/ui` primitives they compose from (NEVER modify a shared primitive for one feature — add a cva variant / opt-in prop, or compose). Forms (RHF + Zod where needed). Expo Router routes affected (thin route files). The generated-client hooks consumed (TanStack Query) + Zustand local state. Loading / error / empty states. Toast + inline error wiring — user-facing text from the **problem+json**-typed error via the global error boundary, never a raw string. **Semantic tokens** used (never raw hex). Accessibility (keyboard / focus on web+desktop, screen-reader labels, focus management).
