...
- @packages/ui/CLAUDE.md + @packages/ui/FIGMA.md — design-system runbook + token contract (semantic tokens only, `@the777incident/ui` primitives, never modify a shared primitive).
...
1. Confirm the target is reachable: try `mcp__playwright__browser_navigate` to the product's **Expo web dev server** — typically `http://localhost:8081` (Expo/Metro web — 8081 for every product; Expo auto-offers the next port when busy), or whatever URL the user instruction specifies. **NOT `:3000`.** If unreachable, STOP and ask the user — do not silently try alternative URLs or environments. (Starting the dev server is the product's `/dev` command — `pnpm turbo run dev --filter=*<product>-*`; if it's not running, tell the user.)
...
4. **After each meaningful step**, capture state:
   ...
   - `browser_take_screenshot` — visual proof. Save into `.playwright-mcp/` (gitignored — never committed). Take screenshots for: every test case's terminal state, every failure, every visual regression candidate.
...
- **No deep refactors / Alembic schema changes / new endpoints / new integrations / changes to shared `@the777incident/ui` primitives or `packages/core` / commonification during this pass.** Those are STOP-and-surface-to-user moments, not silently attempt.
- **Adherence is non-negotiable on every fix** — `@the777incident/ui` primitives + semantic tokens (never hex) + cross-target decisions on the frontend; layered services + DTO≠ORM + problem+json + Alembic-for-schema on the backend; never hand-edit the generated client.
- **Screenshots only into `.playwright-mcp/`** (gitignored). Never commit them.
