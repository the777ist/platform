...
- **Feasibility risk** — can it be built with what we have? (Light touch here — deep feasibility is `/ptfm-architect`'s call; just flag anything that looks expensive or impossible, e.g. needs a brand-new `@the777incident/ui` primitive, a new external integration, or schema changes beyond the existing API.)
...
- **NO ARCHITECTURE / IMPLEMENTATION DECISIONS.** This brief says WHAT and WHY and FOR WHOM. It does NOT say HOW (which framework, which schema, which API endpoint, which `@the777incident/ui` primitives, which layered service). That's `/ptfm-architect`'s job. Stay above the technical-decision line.
