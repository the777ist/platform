...
- **Feasibility risk** — can it be built with what we have? (Light touch here — deep feasibility is `/ptfm-architect`'s call; just flag anything that looks expensive or impossible, e.g. needs a brand-new `@sevenfold/ui` primitive, a new external integration, or schema changes beyond the existing API.)
...
- **NO ARCHITECTURE / IMPLEMENTATION DECISIONS.** This brief says WHAT and WHY and FOR WHOM. It does NOT say HOW (which framework, which schema, which API endpoint, which `@sevenfold/ui` primitives, which layered service). That's `/ptfm-architect`'s job. Stay above the technical-decision line.
...
- **Deployment context** — this product ships to four surfaces (Fly = api, EAS = mobile, Vercel = web, Electron = desktop), infra named `sevenfold-<product>-<env>`. Only reference this if the brief needs to ground a constraint in an existing deployment / integration surface; it is not a workflow pillar for product scoping.
