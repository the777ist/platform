...
- **Frontend STRICTLY conforms to the design system.** Every UI primitive comes from **`@the777incident/ui`** (compose them). Every colour, spacing, radius comes from **NativeWind semantic tokens** (`bg-background`, `bg-primary`, `text-foreground`, `border-border`, …) — **NEVER raw hex**; brand is a token **mode**, and token values come from the Figma pipeline (`theme.ts` / `global.css`), never hand-named. **NEVER modify a shared `@the777incident/ui` primitive to suit one feature** — add a cva variant, add an opt-in prop, or compose on top.
...
- Tables / sub-tables of new files grouped by role (screens / routes, features, `@the777incident/ui` compositions, `packages/core` plumbing, schemas, routers, services, models, Alembic migrations, generated-client hooks, stores, tests). Match the plan's structure where helpful.
...
- **Frontend STRICTLY conforms to the design system.** `@the777incident/ui` primitives only. NativeWind semantic tokens only (never hex). Never reskin a shared primitive for one feature — cva variant / opt-in prop / compose.
