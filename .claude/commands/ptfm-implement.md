...
- Tables / sub-tables of new files grouped by role (screens / routes, features, `@sevenfold/ui` compositions, `packages/core` plumbing, schemas, routers, services, models, Alembic migrations, generated-client hooks, stores, tests). Match the plan's structure where helpful.
...
- **Frontend STRICTLY conforms to the design system.** `@sevenfold/ui` primitives only. NativeWind semantic tokens only (never hex). Never reskin a shared primitive for one feature — cva variant / opt-in prop / compose.
