...
3. **Walk outward for each change**: what calls this code, what this code calls, the types it depends on, the tests that cover it, the feature's `index.ts` public surface. A changed endpoint means reading its router, its service method(s), its Pydantic `schemas/` DTOs, its SQLModel `models/`, its RLS posture, its generated-client hook + the screen that consumes it, and its tests. A changed screen means reading the feature module, the `@the777incident/ui` primitives + `packages/core` helpers it depends on, and its RNTL tests.
...
- **UI composes `@the777incident/ui` primitives only** — no raw React Native `<Pressable>` / `<TextInput>` / `<Text>` re-implementing a primitive that already exists; **no modifying a shared primitive to suit one call site** (extend with an opt-in prop / cva variant / compose). Tier-2 product compositions live in the feature; promote to `packages/ui` on the 2nd use.
...
- **Package + feature-module boundaries** — import a feature via its `index.ts` only, never a deep path into another feature's internals; respect `@the777incident/ui` / `@the777incident/core` / `@the777incident/config` package boundaries; no cross-feature or cross-package deep imports. Cross-PRODUCT sharing of API (Python) code is NOT a locked pattern — flag it and surface, don't bless it.
- File size / complexity kept proportionate; one logical component family per file (reviewer judgment — not linter-enforced).
...
- **Figma** (`mcp__Figma__*`) — this project has a deep Figma integration (Code Connect + token modes). Use it to corroborate that a touched `@the777incident/ui` primitive / token mode (light/dark × brand) matches its Figma component when a finding turns on design-system fidelity. Full UI testing is `/ptfm-test-ui`'s job.
