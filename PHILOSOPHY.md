...
- **Monorepo:** pnpm workspaces + Turborepo 2.10 (`--affected`); mise pins **Node 24 LTS** / **pnpm 11** / Python 3.13 / uv. **pnpm 11 config note:** settings relocate out of `.npmrc` into `pnpm-workspace.yaml` as camelCase keys (`nodeLinker: hoisted`, `preferFrozenLockfile: true`); `.npmrc` is auth/registry-only; `only-built-dependencies` is removed → use the `allowBuilds` map (pnpm 11 requires Node ≥ 22.13, satisfied by 24)
- **Frontend:** Expo SDK 57 (RN 0.86, React 19.2, New Arch mandatory, Hermes v1) + react-native-web, managed + EAS; **NativeWind v4** (v5 is pre-release — do NOT use) on **Tailwind CSS v3** (NOT v4 — v4's CSS-first config belongs to NativeWind v5); Expo Router; **TanStack Query v5 + Zustand v5**; per-platform overrides via `.ios/.android/.web/.native` extensions. **NativeWind-v4 ↔ SDK 57 is unverified by an official pairing (last on-record official pairing is SDK 54); the safe-harbor fallback is SDK 54, NOT 55. Policy: track the current SDK (57 as of 2026-08), fall back to 54 only if it proves unworkable — do not downgrade preemptively.** **SDK 56+ routes global `fetch` through `expo/fetch`** (affects the generated client transport + Sentry network breadcrumbs; escape hatch `EXPO_PUBLIC_USE_RN_FETCH=1`). **EAS Update requires `updates.url` (`https://u.expo.dev/<projectId>`) + a `runtimeVersion` policy in `app.config.ts`** (projectId alone won't deliver OTA)
...
- **Quality:** ESLint flat config + Prettier; Ruff; **pyright strict** + Pydantic strict mode (Python); **single Jest runner** (jest-expo preset) + RNTL for ALL JS tests; Playwright (web E2E, **nightly CI**); Maestro (mobile E2E, **local-only initially**); pytest + httpx against **real Postgres**; typegen drift check; **squawk migration lock-safety lint** (`check-migration-safety.mjs` over `alembic upgrade head --sql`, pre-push + CI — migrations run against production as a Fly release_command, and an empty test DB makes every DDL look safe; `env.py` sets `lock_timeout`/`statement_timeout` in both modes and the lint enforces their presence); GitHub Actions (affected-only) — full inventory in “Testing strategy”
- **Cross-cutting:** Sentry (`@sentry/react-native` — NOT deprecated `sentry-expo`; the **Expo config plugin is `@sentry/react-native/expo`** + Metro `getSentryExpoConfig` wiring for production source maps; pin a release that lists SDK 57 / RN 0.86 support — and it must live in `expo.install.exclude`, because Expo's own bundledNativeModules pins a stale major that `expo install --fix` would silently downgrade to), Expo Push, Supabase Storage/CDN
...
- **Git hooks (Lefthook, repo-level + affected-scoped):** `lefthook.yml` at root. **pre-commit** (fast, staged files only): Prettier + ESLint on staged JS/TS, Ruff check+format on staged `.py` (scoped to the touched product's api). **pre-push:** `turbo run typecheck test build --affected` + (for affected APIs) pyright strict + pytest — i.e. ONLY the product(s) actually touched run (plus all dependents when `packages/*` change, which is the co-evolve guard moved before the push). Builds are turbo-cached so repeat pushes are fast. **Amended (hooks rework):** the tiers now carry explicit BUDGETS — pre-commit ~5s, commit-msg ~0s, pre-push seconds-to-minutes (scoped), CI unbounded — and a gate lives at the earliest tier whose budget it fits. pre-commit is `piped` (semantic fixers, then formatters, so the formatter gets the last write) instead of one parallel block whose globs overlapped on the same files; a **commit-msg** tier runs commitlint (Conventional Commits); pre-push additionally runs `lint` (the real escalation from commit's staged-lines lint), the **typegen drift check** (`node scripts/check-typegen-drift.mjs` — the same checker CI runs; the `git diff --exit-code` it replaced was INERT in the hook, because a quoted wildcard pathspec matches the directory and never its contents) and a one-alembic-head check, scopes its turbo selection to the **remote sha git hands the hook on stdin** (`use_stdin: true` + a `...[base...HEAD]` filter, since `--affected` silently drops `--filter`) rather than a guessed base, and runs `build` INCLUDING `desktop#build` → `^export:web` (a real `expo export`, ~83s cold per affected product) — the budget was raised rather than the coverage dropped, because a Metro bundle fails in ways no type check can see. pyright moved out of each api's `lint` script into a `typecheck` script so `turbo run typecheck --affected` actually enforces it at pre-push, as this sheet already claimed it did
- **Design system workbench:** **Storybook 10 (10.5.x)** (web, **`@storybook/react-native-web-vite`** — renders the SAME RN components through react-native-web that ship to every target; NOT on-device `@storybook/react-native`; started on 9 while 10's ESM-only move was fresh, moved to 10 once the RN-web-vite framework proved VR-identical on it; **Vite stays on 7** — Vite 8's Rolldown core cannot resolve tsconfigs for the RN files the framework transforms out of node_modules) as a SINGLE shared workbench in `packages/ui` — `.storybook/main.ts` sets `jsxImportSource: "nativewind"` and runs the Tailwind/NativeWind step on `global.css`; the framework already aliases `react-native`→`react-native-web` (no manual alias needed) — stories colocated (`*.stories.tsx`, one story per cva variant), run with `pnpm --filter @platform/ui storybook`. Global decorator imports `global.css` + wraps in the theme provider so `className`/NativeWind utilities resolve identically to the app; toolbar exposes a **light/dark toggle AND a brand switcher** (template ↔ demo) that swaps the active CSS-var set — so the workbench is also the live preview surface for the Figma token modes (below) and the demo-able "one component set, different brand" moment. Visual regression = Playwright screenshots of the static Storybook build (iterates the stories `index.json`, each story × {light,dark}), committed baselines, wired into the nightly E2E run. **Chromatic deliberately declined** — self-hosted Playwright keeps VR free + in-repo, consistent with the no-paid-SaaS stance elsewhere
...
- **Cloud dev workstation (devbox):** one Fly machine + one volume per seat, committed under `devbox/` — the IMAGE is the toolchain (docker-ce for the Supabase stacks, mise-pinned node/pnpm/python/uv mirroring mise.toml, Supabase CLI, gh, tmux, Claude Code; rebuilt on `devbox deploy`) and the VOLUME is the identity (`/data` holds HOME — Claude/gh/MCP auth, clones, pnpm store — plus dockerd's data-root, so pulled stack images survive redeploys). NO services exposed: access is `fly ssh console` + tmux, and a service-less machine is never auto-stopped — the lifecycle is manual by decision (`scripts/devbox.mjs up|down|ssh`; no idle watchdog). Org-parameterized like all infra (`example-devbox` placeholder; the wrapper REFUSES to operate until it is swapped per clone, and app-name uniqueness is manual — check-product-identity does not scan it). Team = one seat per person (`<org>-devbox-<name>`), auth per-HOME hence per-seat by construction
...
- **DB conventions (template defaults):** **UUIDv7 PKs** (SQLModel base model) — generated via the maintained **`uuid-utils`** (`from uuid_utils import uuid7`) or `uuid6`, pinned exact (**app-side generation — PG-version-independent; deliberately NOT Postgres-native `uuidv7()`**, so the template runs on any Supabase Postgres); NOT the unmaintained `uuid7` PyPI package, and stdlib `uuid.uuid7` only lands in Python 3.14. **`DELETE`/`UPDATE` go through `session.execute(delete(...))`, never `session.exec(...)`** (SQLModel `exec()` only types `select()` — `delete()`/`update()` break pyright strict and don't expose `.rowcount`); **RLS deny-all on every table** via the template's initial migration (the API's privileged role bypasses it; PostgREST/Realtime surface locked, opened per-table only where Realtime reads are wanted); schema changes ONLY via Alembic
...
- **Observability:** Sentry + **structlog JSON logs** + `request_id` middleware in FastAPI; the API-client wrapper sends a generated `X-Request-Id` per request; Sentry events tagged with it on both sides → client→API→logs traceability
- **Agentic tooling (committed MCP surface):** the root **`.mcp.json`** is the machinery form of "connect these first" — every clone inherits the pipeline's tool servers instead of reading a table and wiring them by hand. Committed set: **Sentry** (hosted, OAuth — the read side of the X-Request-Id observability chain, which was otherwise write-only from the pipeline's perspective), **Fly** (`flyctl mcp server`, local CLI auth — migrations run as the Fly release_command, so failed-migration logs live where only this can reach), **Expo/EAS** (hosted, OAuth — builds, workflows, TestFlight crashes; mobile's only production signal since Maestro is local-only), **Vercel** (hosted, OAuth — the web surface deliberately has NO deploy workflow, so this is its only pipeline-visible telemetry), **Semgrep** (`uvx semgrep-mcp`, free OSS engine — the deterministic SAST floor under `/ptfm-review`'s model-judgement security pass), **Chrome DevTools** (`npx chrome-devtools-mcp` — perf traces / Core Web Vitals for `/ptfm-test-ui`; Playwright verifies behaviour, nothing else measures speed). User-connected (authenticate in Claude Code; unauthenticated = absent): Linear, Notion, Figma, Supabase, Playwright, GitHub. **Chromatic stays declined** (self-hosted Playwright VR); no paid-SaaS dependencies were added — every committed server is free-tier or local-CLI.
...
├── lefthook.yml                   # tiered hooks + the budgets (pre-commit / commit-msg / pre-push)
├── .lefthook/                     # hook bodies as real scripts (ruff.sh, pre-push.sh)
├── commitlint.config.mjs          # Conventional Commits, header <=120
...
`turbo run openapi build --filter=*api-client* && node scripts/check-typegen-drift.mjs`.
...
4. Ports from portIndex: API `apiBase+10i`; Supabase block `sbBase+100i` (bases per-REPO in root `platform.json`, defaults 8000/54321 — `scripts/set-port-base.mjs` rebases an org-repo so several stamped repos' stacks coexist on one machine; portIndex alone only de-conflicts within a repo) → products' local stacks coexist. Applied everywhere ports appear: supabase `config.toml`, api dev script, AND the committed `app/.env.*` files (`EXPO_PUBLIC_API_URL`, supabase URL).
...
| Contract (API ↔ generated client can't drift)                                                                   | regen `openapi.json` + client → `node scripts/check-typegen-drift.mjs`                                                                                              | CI step                                                                       | every PR                                                                      |
