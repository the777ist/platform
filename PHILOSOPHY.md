...
- **Git hooks (Lefthook, repo-level + affected-scoped):** `lefthook.yml` at root. **pre-commit** (fast, staged files only): Prettier + ESLint on staged JS/TS, Ruff check+format on staged `.py` (scoped to the touched product's api). **pre-push:** `turbo run typecheck test build --affected` + (for affected APIs) pyright strict + pytest — i.e. ONLY the product(s) actually touched run (plus all dependents when `packages/*` change, which is the co-evolve guard moved before the push). Builds are turbo-cached so repeat pushes are fast. **Amended (hooks rework):** the tiers now carry explicit BUDGETS — pre-commit ~5s, commit-msg ~0s, pre-push ~90s, CI unbounded — and a gate lives at the earliest tier whose budget it fits. pre-commit is `piped` (semantic fixers, then formatters, so the formatter gets the last write) instead of one parallel block whose globs overlapped on the same files; a **commit-msg** tier runs commitlint (Conventional Commits); pre-push additionally runs `lint` (the real escalation from commit's staged-lines lint), the **typegen drift check** (`git diff --exit-code` on the regenerated client + openapi.json — the same command CI runs) and a one-alembic-head check, scopes its turbo selection to the **remote sha git hands the hook on stdin** (`use_stdin: true` + a `...[base...HEAD]` filter, since `--affected` silently drops `--filter`) rather than a guessed base, and omits `build` as a top-level task (`desktop#build` → `^export:web` cannot fit a 90s tier, and nothing depends on it — while `api-client#build` still arrives via `typecheck`'s `^build` edge, so desktop keeps its lint/typecheck and the drift check keeps its regenerated artifacts). pyright moved out of each api's `lint` script into a `typecheck` script so `turbo run typecheck --affected` actually enforces it at pre-push, as this sheet already claimed it did
