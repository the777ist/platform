...
StyleDictionary.registerFormat({
  ...
  format: ({ dictionary }) => {
    ...
    // Ready-to-use colour STRINGS alongside the vars() objects. `vars()` returns an OPAQUE
    // NativeWind style object — you cannot read a token back out of it — but some surfaces are
    // not NativeWind surfaces at all: React Navigation chrome (the tab bar) takes plain colour
    // props and `className` never reaches it, so without these it renders React Navigation's
    // own light palette under a dark app. Hardcoding a hex there would break tokens-only.
    //
    // The hsl() wrapper is applied HERE rather than at the call site for two reasons: consumers
    // want a usable colour string, not a bare "0 0% 100%" triple; and it keeps every component
    // free of a literal `hsl(`, which is exactly what scripts/check-semantic-tokens.mjs scans
    // for. Exempting that guard was the alternative — and exemptions are how a guard quietly
    // stops covering things. Both exports come from the SAME resolved tree, so they cannot drift.
    ...
    return (
      ...
      `export const themeColors = {\n${MODE_ORDER.map(colorBlock).join("\n")}\n} as const;\n\n` +
