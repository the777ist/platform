...
// baselines. Build first: pnpm --filter @sevenfold/ui build-storybook
// Author/refresh baselines: pnpm --filter @sevenfold/ui exec playwright test --update-snapshots
...
if (!fs.existsSync(indexPath)) {
  throw new Error(
    "storybook-static/index.json not found — run `pnpm --filter @sevenfold/ui build-storybook` first.",
  );
}
