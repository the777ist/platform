...
module.exports = {
  ...
  // the packages this stack adds (@rn-primitives, nativewind, css-interop, cva, @sevenfold).
  // Re-verify when a new ESM dep lands ("Cannot use import statement outside a module").
  transformIgnorePatterns: [
    "node_modules/(?!((jest-)?react-native|@react-native(-community)?|expo(nent)?|@expo(nent)?/.*|@expo-google-fonts/.*|react-navigation|@react-navigation/.*|@sentry/react-native|native-base|react-native-svg|@rn-primitives/.*|nativewind|react-native-css-interop|class-variance-authority|@sevenfold/.*))",
  ],
};
