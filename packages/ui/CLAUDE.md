# @the777incident/ui — design-system runbook
...
1. **Semantic tokens ONLY — never name a color.** Components use `bg-primary`,
   `text-foreground`, `border-border`, … (the `@the777incident/config/tailwind-preset` maps them to
...
1. `cli-add` (or author by hand): `pnpm --filter @the777incident/ui dlx @react-native-reusables/cli add <name>`,
...
6. Commit VR baselines (light + dark, PER PLATFORM — font rendering differs across OSes):
   `pnpm --filter @the777incident/ui build-storybook` then
   `pnpm --filter @the777incident/ui exec playwright test --update-snapshots` produces YOUR
...
- `pnpm --filter @the777incident/ui storybook` → http://localhost:6006 (app stays on 8081).
...
- `pnpm --filter @the777incident/ui build-storybook` → `storybook-static/` (+ `index.json`, consumed
